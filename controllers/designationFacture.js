...
exports.getdesignations = async (req, res) => {
  try {
    let range = req.query.range || "[0,50]";
    ...
    res.set(
      "Content-Range",
      `designation ${range[0]}-${range[1] + 1 - range[0]}/${req.count}`
    );
...
exports.getdesignationbyid = async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .query(designation.getdesignationbyid);

    res.set("Content-Range", `designation 0-1/1`);
    res.json(result.recordset);
  } catch (error) {
    res.send(error.message);
    res.status(500);
